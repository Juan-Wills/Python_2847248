...
from services.biblioteca_service import validate_input_format, validate_menu_options, autoincrement
...
def gestion_libros():
    ...
    def get_feat(option):
        try:
            result= book_features[int(option)-1]
        except (IndexError, ValueError):
            print("Error: Opcion no valida, por favor ingrese un numero dentro del rango de opciones.")
            return False
        return result
            ...
            match option:
                ...
                case '1':
                    while True:
                        ...
                        print('1. Titulo    2. Autor    3. Genero    4. Editorial    5. Fecha    6. Id    7. Todo')
                        option= input('Ingresar opcion: ')

                        if option == '7':
                            data= bs.retrieve_data(bs.libro_path)
                            for book in data['libros']:
                                print(book)
                            break

                        if not validate_menu_options(option, get_feat(option), type='category', max_args=7):
                            continue

                        filter= input("Buscar: ").title().strip()
                        if books:= bs.find_book(get_feat(option), filter):
                            for book in books:
                                print(book)
                            break
                ...
                case '2':
                    ...
                    new_libro= Libro(autoincrement(), titulo, genero, autor, editorial, publicacion)
                    print(f"\nVista previa:\n{new_libro}")
                ...
                case '3':
                    while True:
                        ...
                        if not validate_menu_options(menu_option, mode= "equal", max_args=6, min_args=1): 
                            continue

                        busqueda= input("Buscar: ")

                        book= bs.find_book(get_feat(menu_option), busqueda)

                        if not book:
                            print("No se encontraron libros con la busqueda proporcionada. Por favor, verifique los datos e intente nuevamente.")
                            continue
                        ...
                        while True:
                            ...
                            if not validate_menu_options(modificar, input= get_feat(modificar), type= 'category', max_args=6):
                                continue

                            attribute= get_feat(modificar)
                            ...
                            while True:
                                ...
                                if not validate_menu_options(save, mode= "equal", max_args=2, min_args=1):
                                    continue
                ...
                case '4':
                    while True:
                        data= bs.retrieve_data(bs.libro_path)
                        print("\nEliminar libro")
                        id_book= input("Ingrese los Id de los libros a eliminar (separados por comas): ").replace(" ", "").split(',')

                        found_books= []
                        try:
                            for id in id_book:
                                for book in bs.find_book('id', id.strip()):
                                    found_books.append(book)
                        except TypeError:
                            print("Error: Para eliminar varios libros, se deben ingresar los Id separados por comas.")
                            continue                            
                        ...
                        while True:
                            ...
                            if len(id_book) == 1:
                                bs.del_book(id_book[0], mode='single', sort=True)

                            else:
                                bs.del_book(id_book, mode='multi', sort=True)
